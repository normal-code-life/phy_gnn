...
class FEHeartSageV3Trainer(BaseTrainer):
    ...
    def compute_loss(self, predictions: Dict[str, torch.Tensor], labels: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        losses = dict()
...
class FEHeartSAGEModel(BaseModule):
    ...
    def _edge_coord(self, input_node_coord: torch.Tensor, input_edge_indices: torch.Tensor, selected_node: torch.Tensor) -> torch.Tensor:
    ...
    def random_select_nodes(self, indices: torch.Tensor, select_node: torch.Tensor) -> torch.Tensor:
        ...
        select_indices = torch.randint(0, seq_num, (batch_size, select_node_num, self.select_edge_num), dtype=torch.int64)

        selected_edge = indices[select_batch[:, None, None], select_node[None, :, None], select_indices]  # shape: (batch_size, node_num, seq)
