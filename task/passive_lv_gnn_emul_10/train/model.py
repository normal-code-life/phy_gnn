...
from task.passive_lv_gnn_emul_10.train.datasets import LvDataset
from task.passive_lv_gnn_emul_10.train.message_passing_layer import MessagePassingModule
from task.passive_lv_gnn_emul_10.train.mlp_layer_ln import MLPLayerLN
...
class PassiveLvGNNEmulTrainer(BaseTrainer):
    ...
    def create_model(self) -> BaseModule:
        return PassiveLvGNNEmulModel(
            self.task_train, self.senders, self.receivers, self.n_total_nodes
        )
...
class PassiveLvGNNEmulModel(BaseModule):
    ...
    def forward(self, x):
        ...
        node, edge = self.message_passing_layer(node, edge, input_node)  # shape: (126, 40), (440, 40)
