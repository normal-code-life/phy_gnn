...
class LvDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        # test: add a single node to let all the other node connect to this node
        sequence = np.arange(1, 96)
        zeros_vector = np.zeros((95, 1), dtype=int)
        single_topology_indices = np.hstack((sequence[:, np.newaxis], zeros_vector))

        topology_indices = np.concatenate((real_topology_indices, single_topology_indices), axis=0)
        topology_indices = np.unique(topology_indices, axis=0)

        self._senders = topology_indices[:, 0]
        self._receivers = topology_indices[:, 1]

        # ==== calculate edge features
        edge = self.generate_topology_data(topology_indices, node_coords)

        # === calculate edge distance
        edge_distance = np.expand_dims(np.sqrt((edge ** 2).sum(axis=2)), axis=2)

        self._edges = torch.from_numpy(
            np.concatenate(
                (self.coord_normalization_max_min(edge), self.distance_normalization_max_min(edge_distance)
                 ),
                axis=2
            )
        )
