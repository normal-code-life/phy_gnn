...
class MessagePassingModule(BaseModule):
    ...
    def message_passing_block(self, node, edge, input_node, i):
        ...
        edge_input_receivers = torch.concat((edge, input_node[senders], input_node[receivers]), dim=-1)  # shape: (440, 80)
        edge_input_senders = torch.concat((-edge, input_node[receivers], input_node[senders]), dim=-1)  # shape: (440, 80)
